...
export default class Okex extends WS {
  static apiSocketUrl: string = 'wss://real.okex.com:10441/websocketx';
  static apiRestUrl: string = 'https://www.okex.com/api/v1/';
  fnQueue: object = {};

  constructor(private readonly api_key: string, private readonly secret_key: string, wsOpts?: any) {
  ...
  wsAddChanel(channel: string, parameters?: any, fn?, event = 'addChannel') {
  ...
  async getData(url: string, data: any = {}, method: string = 'get') {
    ...
    return await Req.getJson(url, data, method);
  }
  ...
  doTrade(params: {}) { // 买卖类型：限价单(buy/sell) 市价单(buy_market/sell_market); 下单价格 市价卖单不传price; 交易数量 市价买单不传amount
